...
function filtered(){const terms=state.q.trim().toLowerCase().split(/\s+/).filter(Boolean);return DATA.filter(x=>{if(state.track&&x.track!==state.track)return false;if(state.year&&String(x.year)!==state.year)return false;if(state.type&&x.type!==state.type)return false;if(!hasState(x,state.pageState))return false;if(state.hot&&!x.topics.some(t=>t.label===state.hot))return false;if(terms.length){const hay=[x.id,x.year,x.track,x.number,x.primary_topic,...x.topics.map(t=>t.label)].join(" ").toLowerCase();if(!terms.every(term=>termMatches(x,hay,term)))return false}return true})}
