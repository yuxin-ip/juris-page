...
/* Each selected subject keeps its own two-level topic filter. */
.subject-panel{margin-top:9px;padding:9px 10px;border:1px solid var(--line);border-radius:11px;background:#fff}.subject-panel.active{border-color:#bfd3fb;background:var(--blue-soft)}.subject-switch{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;width:100%}.subject-button{width:100%;min-width:0;min-height:42px;border:1px solid var(--line);border-radius:8px;background:#f8fafc;color:var(--text);font:inherit;font-size:14px;font-weight:650;cursor:pointer}.subject-button.active{border-color:var(--blue);background:var(--blue);color:#fff;box-shadow:0 2px 6px rgba(35,87,166,.2)}.filters{grid-template-columns:repeat(3,minmax(0,1fr))}.topic-filter-groups{grid-column:1/-1;display:grid;gap:8px;margin-left:auto}.topic-filter-groups.single{grid-template-columns:minmax(0,1fr);width:calc(33.333% - 6px)}.topic-filter-groups.dual{grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.dual-select{display:grid;grid-template-columns:1fr;gap:5px}.dual-select span{padding:0}select.long-value{font-size:11px}select.xlong-value{font-size:10px}.end-hint{text-align:center;margin:18px auto 0;color:var(--muted);font-size:12px}
@media(max-width:680px){.filters{grid-template-columns:repeat(3,minmax(0,1fr))}.filter-group span{font-size:10px}.topic-filter-groups.single{width:calc(33.333% - 5px)}.topic-filter-groups.dual{width:100%}}
.topic-filter-groups.single{width:100%}
</style>
<style>
#topicFilterGroups[hidden],#criminalTopicFilter[hidden],#civilTopicFilter[hidden]{display:none!important}
...
.hero-top{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.hero-top h1{margin:0 0 6px}.about-toggle{flex:none;border:1px solid #c7d8f6;border-radius:8px;background:#fff;color:#2357a6;padding:5px 9px;font:inherit;font-size:12px;font-weight:650;cursor:pointer}.about-panel{margin-top:8px;padding:9px 10px;border:1px solid #d9e5fa;border-radius:9px;background:#f8fbff;color:#4a5c78;font-size:12px}.about-panel p{margin:3px 0}.back-to-top{display:grid;place-items:center;width:48px;height:48px;min-width:0;position:fixed;right:16px;bottom:20px;z-index:30;margin:0;padding:0;border:1px solid #9dbbf2;border-radius:50%;box-sizing:border-box;overflow:hidden;background:rgba(255,255,255,.96);box-shadow:0 3px 12px rgba(35,87,166,.18);color:#17458a;font-size:8px!important;font-weight:650;line-height:1;cursor:pointer}.back-to-top[hidden]{display:none!important}.back-to-top span{display:block;white-space:nowrap;letter-spacing:-.35px;text-align:center}.back-to-top.armed{border-color:var(--blue);background:var(--blue);color:#fff}
</style>
</head>
...
</section><div class="sticky-filters"><div class="filters" id="filters"><label class="filter-group"><span>类别 · 法学 / 非法学</span><select id="track" aria-label="考生类别"><option value="">全部</option><option value="非法学">非法学</option><option value="法学">法学</option></select></label><label class="filter-group"><span>年份</span><select id="year" aria-label="年份"><option value="">全部年份</option></select></label><label class="filter-group"><span>题型 · 单选 / 多选</span><select id="type" aria-label="题型"><option value="">全部</option><option value="single">单选</option><option value="multiple">多选</option></select></label><div class="topic-filter-groups" id="topicFilterGroups" hidden><label class="filter-group" id="criminalTopicFilter" hidden><span>刑法分则罪名</span><span class="dual-select"><select id="criminalTopicCategory" aria-label="犯罪类型"></select><select id="criminalTopic" aria-label="具体罪名"></select></span></label><label class="filter-group" id="civilTopicFilter" hidden><span>民法编号知识点</span><span class="dual-select"><select id="civilTopicCategory" aria-label="民法典全部七编"></select><select id="civilTopic" aria-label="具体知识点"></select></span></label></div></div><div class="filter-state" id="filterState" aria-live="polite" hidden><span>当前已启用筛选条件，搜索结果可能减少。</span><button id="resetFilters" class="reset-filters" type="button">重置筛选</button></div></div>
...
<section class="list" id="list" aria-live="polite"></section><button id="load" class="load" hidden>显示更多</button><div id="endHint" class="end-hint" aria-live="polite" hidden></div><button id="backToTop" class="back-to-top" type="button" aria-label="返回顶部" hidden><span id="backToTopText">返回顶部</span></button>
...
const DATA=__DATA__;const TOPIC_FILTERS=__TOPIC_FILTERS__;const PAGE_SIZE=80;let visible=PAGE_SIZE;const state={q:"",track:"",year:"",type:"",subjects:[],criminalCategory:"",criminalTopic:"",civilCategory:"",civilTopic:""};
...
function subjectTopicState(subject){return subject==='刑法'?{category:state.criminalCategory,topic:state.criminalTopic}:{category:state.civilCategory,topic:state.civilTopic}}
function filtered(){const terms=state.q.trim().toLowerCase().split(/\s+/).filter(Boolean);return DATA.filter(x=>{if(state.track&&x.track!==state.track)return false;if(state.year&&String(x.year)!==state.year)return false;if(state.type&&x.type!==state.type)return false;if(state.subjects.length===1&&!state.subjects.includes(x.subject))return false;if(state.subjects.includes(x.subject)){const current=subjectTopicState(x.subject);const categoryTopics=new Set((TOPIC_FILTERS.find(item=>item.subject===x.subject&&item.label===current.category)||{topics:[]}).topics);if(current.category&&!x.topics.some(t=>categoryTopics.has(t.label)))return false;if(current.topic&&!x.topics.some(t=>t.label===current.topic))return false}if(terms.length){const hay=[x.id,x.year,x.track,x.subject,x.number,x.primary_topic,...x.topics.map(t=>t.label)].join(" ").toLowerCase();if(!terms.every(term=>termMatches(x,hay,term)))return false}return true})}
...
function syncFilterState(){const ids=['track','year','type','criminalCategory','criminalTopic','civilCategory','civilTopic'];const active=Boolean(singleSubject())||ids.some(id=>Boolean(state[id]));$('#filters').classList.toggle('active',active);$('#filterState').hidden=!active;$('#subjectPanel').classList.toggle('active',Boolean(singleSubject()));for(const id of ids){const selectId=id==='criminalCategory'?'criminalTopicCategory':id==='civilCategory'?'civilTopicCategory':id;$('#'+selectId).classList.toggle('active',Boolean(state[id]))}syncSubjectButtons()}
...
function scrollToResults(){requestAnimationFrame(()=>requestAnimationFrame(()=>{const sticky=$('.sticky-filters');const top=$('#list').getBoundingClientRect().top+window.scrollY-(sticky?sticky.offsetHeight:0)-14;window.scrollTo({top:Math.max(0,top),behavior:'smooth'})}))}
...
function availableGroups(subject){return TOPIC_FILTERS.filter(x=>x.subject===subject)}
function topicOptions(subject,category){const item=TOPIC_FILTERS.find(x=>x.subject===subject&&x.label===category);return item?item.topics:availableGroups(subject).flatMap(x=>x.topics)}
function selectTextSize(select,value){select.classList.toggle('long-value',value.length>12&&value.length<=20);select.classList.toggle('xlong-value',value.length>20)}
function fillTopicSelector(subject){const criminal=subject==='刑法',prefix=criminal?'criminal':'civil',selected=state.subjects.includes(subject),group=$('#'+prefix+'TopicFilter'),categorySelect=$('#'+prefix+'TopicCategory'),topicSelect=$('#'+prefix+'Topic');group.hidden=!selected;if(!selected){state[prefix+'Category']='';state[prefix+'Topic']=''}const groups=availableGroups(subject),labels=groups.map(x=>x.label);if(!labels.includes(state[prefix+'Category']))state[prefix+'Category']='';const categoryLabel=criminal?'犯罪类型':'民法典全部七编',topicLabel=criminal?'具体罪名':'具体知识点';categorySelect.innerHTML=`<option value="">${categoryLabel}</option>`+groups.map(x=>`<option value="${esc(x.label)}">${esc(x.label)}</option>`).join('');categorySelect.value=state[prefix+'Category'];const options=topicOptions(subject,state[prefix+'Category']);if(!options.includes(state[prefix+'Topic']))state[prefix+'Topic']='';topicSelect.innerHTML=`<option value="">${topicLabel}</option>`+options.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');topicSelect.value=state[prefix+'Topic'];selectTextSize(categorySelect,state[prefix+'Category']);selectTextSize(topicSelect,state[prefix+'Topic'])}
function fillTopicSelectors(){const group=$('#topicFilterGroups'),count=state.subjects.length;group.hidden=count===0;group.classList.toggle('single',count===1);group.classList.toggle('dual',count===2);fillTopicSelector('刑法');fillTopicSelector('民法')}
fillTopicSelectors();
$('#search').addEventListener('input',e=>{state.q=e.target.value;resetRender()});for(const id of ['track','year','type']){$('#'+id).addEventListener('change',e=>{state[id]=e.target.value;resetRender(true)})}for(const subject of ['刑法','民法']){const prefix=subject==='刑法'?'criminal':'civil';$('#'+prefix+'Topic').addEventListener('change',e=>{state[prefix+'Topic']=e.target.value;selectTextSize(e.target,e.target.value);resetRender(true)});$('#'+prefix+'TopicCategory').addEventListener('change',e=>{state[prefix+'Category']=e.target.value;state[prefix+'Topic']='';fillTopicSelector(subject);resetRender(true)})}document.querySelectorAll('[data-subject]').forEach(button=>button.addEventListener('click',()=>{const subject=button.dataset.subject;state.subjects=state.subjects.includes(subject)?state.subjects.filter(x=>x!==subject):[...state.subjects,subject];fillTopicSelectors();resetRender(true)}));$('#resetFilters').addEventListener('click',()=>{for(const id of ['track','year','type','criminalCategory','criminalTopic','civilCategory','civilTopic'])state[id]='';state.subjects=[];for(const id of ['track','year','type'])$('#'+id).value='';fillTopicSelectors();resetRender(true)});
let backToTopTimer;function syncBackToTop(){const button=$('#backToTop');button.hidden=window.scrollY<560;if(button.hidden){button.classList.remove('armed');$('#backToTopText').textContent='返回顶部'}}window.addEventListener('scroll',syncBackToTop,{passive:true});$('#backToTop').addEventListener('click',()=>{const button=$('#backToTop'),text=$('#backToTopText');if(!button.classList.contains('armed')){button.classList.add('armed');text.textContent='再点确认';clearTimeout(backToTopTimer);backToTopTimer=setTimeout(()=>{button.classList.remove('armed');text.textContent='返回顶部'},2200);return}clearTimeout(backToTopTimer);button.classList.remove('armed');text.textContent='返回顶部';window.scrollTo({top:0,behavior:'smooth'})});syncBackToTop();
...
</style></head><body><main class="wrap"><div class="top"><h1>使用说明</h1><a class="back" href="index.html">返回速查</a></div><section class="card note"><h2>资料范围</h2><p>收录 2010—2026 年法律硕士法学、非法学刑法与民法客观题，共 1360 道；其中刑法、民法各 680 道。</p><p>页码对应众合 2027《背诵一本通》《精讲一本通》的纸质书印刷页。</p></section><section class="card"><h2>如何筛选</h2><ul><li>可直接输入年份、题号、科目、考点或罪名，例如“2025”“民法”“诈骗罪”。</li><li>空格可组合筛选，例如“2025 法学 抢劫罪”。</li><li>输入“202506”可同时查看两类试卷的 2025 年第 6 题。</li><li>民法、刑法可分别勾选；两科同时勾选时，可分别使用刑法分则罪名和民法编号知识点筛选。</li></ul></section><section class="card"><h2>使用提示</h2><p>同一道题可能关联多个罪名或知识点；民法仅归入《背诵一本通》中有编号、有标题的知识点。</p><p>本网站不提供教材扫描件、真题题面或第三方解析全文；页码定位建议结合题目解析使用。</p></section><section class="card"><h2>问题反馈</h2><p>微信：zlszyxdwx</p></section><footer class="footer"><p>版权所有 © 2026 郑宇昕</p><p>作者：南京理工大学知识产权学院 郑宇昕</p></footer></main></body></html>'''
