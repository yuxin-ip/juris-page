...
<style>#topicFilterGroup[hidden]{display:none!important}</style>
</head>
...
fillTopicSelectors();
$('#search').addEventListener('input',e=>{state.q=e.target.value;resetRender()});for(const id of ['track','year','type','topic']){$('#'+id).addEventListener('change',e=>{state[id]=e.target.value;resetRender()})}document.querySelectorAll('[data-subject]').forEach(button=>button.addEventListener('click',()=>{const subject=button.dataset.subject;state.subjects=state.subjects.includes(subject)?state.subjects.filter(x=>x!==subject):[...state.subjects,subject];state.topicCategory='';state.topic='';fillTopicSelectors();resetRender()}));$('#topicCategory').addEventListener('change',e=>{state.topicCategory=e.target.value;fillTopicSelectors();resetRender()});$('#resetFilters').addEventListener('click',()=>{for(const id of ['track','year','type','topicCategory','topic'])state[id]='';state.subjects=[];for(const id of ['track','year','type'])$('#'+id).value='';fillTopicSelectors();resetRender()});
