...
.subject-panel{margin-top:9px;padding:9px 10px;border:1px solid var(--line);border-radius:11px;background:#fff}.subject-switch{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;width:100%}.subject-button{width:100%;min-width:0;min-height:42px;border:1px solid var(--line);border-radius:8px;background:#f8fafc;color:var(--text);font:inherit;font-size:14px;font-weight:650;cursor:pointer}.subject-button.active{border-color:var(--blue);background:var(--blue);color:#fff;box-shadow:0 2px 6px rgba(35,87,166,.2)}.filters{grid-template-columns:1.15fr 1fr 1fr 1.45fr}.dual-select{display:grid;grid-template-columns:1fr;gap:5px}.dual-select span{padding:0}.end-hint{text-align:center;margin:18px auto 0;color:var(--muted);font-size:12px}
...
const DATA=__DATA__;const TOPIC_FILTERS=__TOPIC_FILTERS__;const PAGE_SIZE=80;let visible=PAGE_SIZE;const state={q:"",track:"",year:"",type:"",subject:"刑法",topicCategory:"",topic:""};
...
function captions(){return state.subject==='刑法'?['刑法分则罪名','犯罪类型','具体罪名']:['民法编号知识点','民法典全部七编','具体知识点']}
...
fillTopicSelectors();
$('#search').addEventListener('input',e=>{state.q=e.target.value;resetRender()});for(const id of ['track','year','type','topic']){$('#'+id).addEventListener('change',e=>{state[id]=e.target.value;resetRender()})}document.querySelectorAll('[data-subject]').forEach(button=>button.addEventListener('click',()=>{state.subject=button.dataset.subject;state.topicCategory='';state.topic='';fillTopicSelectors();resetRender()}));$('#topicCategory').addEventListener('change',e=>{state.topicCategory=e.target.value;fillTopicSelectors();resetRender()});$('#resetFilters').addEventListener('click',()=>{for(const id of ['track','year','type','topicCategory','topic'])state[id]='';state.subject='刑法';for(const id of ['track','year','type'])$('#'+id).value='';fillTopicSelectors();resetRender()});
